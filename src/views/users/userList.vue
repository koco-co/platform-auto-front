...
<template>
  ...
  <!-- 搜索表单 - 提供用户筛选功能 -->
  <!-- @submit.prevent: 阻止表单的默认提交行为, 使用自定义提交逻辑 -->
  <!-- @keyup.enter="loadData()": 当用户按回车键时，调用 loadData() 方法进行查询  -->
  ...
  >
    ...
    <el-row class="mb-4" type="flex" justify="end">
      ...
      <!-- 新增按钮 - 跳转到用户表单页面 -->
      <el-button type="warning" @click="onDataForm(-1)">新增用户</el-button>
    </el-row>
  </el-form>
  <!-- END 搜索表单 -->
...
import { queryByPage, deleteData } from "./user"; // 导入用户相关的API函数, 不同页面不同的接口
...
// 定义表格数据，用于存储用户列表
const tableData = ref([]); // 使用ref创建响应式数据
